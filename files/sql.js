...
const conString = "postgres://postgres:pass@localhost:5432/testdb";
...
module.exports = {
	...
	searchByTitleApprox: async (title) => {
		let res = await client.query(
			`select title, price, num_pages, year, quantity, publisher_name
	     from book
	     where title ~* '${title}';`
	...
	registerUser: async (newUser) => {
		...
		if (uniqueUser.rows.length > 0) {
			...
			// Creating New Client
			await client.query(
				`insert into client(username, password, email, billing_id, shipping_id)
